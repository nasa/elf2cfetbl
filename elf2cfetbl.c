...
int32 GetElfHeader(void)
{
    ...
    if (Verbose) printf("   e_ident[EI_MAG0..3] = 0x%02x,%c%c%c\n",
                        get_e_ident(&ElfHeader, EI_MAG0), get_e_ident(&ElfHeader, EI_MAG1),
                        get_e_ident(&ElfHeader, EI_MAG2), get_e_ident(&ElfHeader, EI_MAG3));

    /* Verify the ELF file magic number */
    if (get_e_ident(&ElfHeader, EI_MAG0) != ELFMAG0) Status = FAILED;
    if (get_e_ident(&ElfHeader, EI_MAG1) != ELFMAG1) Status = FAILED;
    if (get_e_ident(&ElfHeader, EI_MAG2) != ELFMAG2) Status = FAILED;
    if (get_e_ident(&ElfHeader, EI_MAG3) != ELFMAG3) Status = FAILED;
...
int32 GetSectionHeader(int32 SectionIndex, union Elf_Shdr *SectionHeader)
{
    ...
    if ((SectionHeaderStringTableDataOffset != 0) && (get_sh_name(SectionHeader) != 0))
    {
        ...
        if (Verbose) printf("   sh_name       = 0x%08x - ", get_sh_name(SectionHeader));
        ...
        if (Verbose) print_sh_flags(SectionHeader);
...
int32 GetSymbol(int32 SymbolIndex, union Elf_Sym *Symbol)
{
    ...
    if (Verbose) printf("   st_name  = 0x%08x - ", get_st_name(Symbol));
    fseek(SrcFileDesc, SeekOffset, SEEK_SET);
...
void PrintSymbol32(union Elf_Sym *Symbol)
{
    if (Verbose) printf("   st_value = 0x%x\n", Symbol->Sym32.st_value);
...
void PrintSectionHeader32(union Elf_Shdr *SectionHeader)
{
    if (Verbose) printf("   sh_addr       = 0x%x\n", SectionHeader->Shdr32.sh_addr);
...
int32 GetTblDefInfo(void)
{
    ...
    else
    {
        ...
        uint64_t calculated_offset = get_sh_offset(SectionHeaderPtrs[get_st_shndx(SymbolPtrs[TblDefSymbolIndex])]) + get_st_value(SymbolPtrs[TblDefSymbolIndex]);
...
int32 LocateAndReadUserObject(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            uint64_t calculated_offset = get_sh_offset(SectionHeaderPtrs[get_st_shndx(SymbolPtrs[UserObjSymbolIndex])]) + get_st_value(SymbolPtrs[UserObjSymbolIndex]);
            SeekOffset = (uint32_t) (calculated_offset);
            if (SeekOffset != calculated_offset)
            {
                printf("Error: SeekOffset may not be %lu\n", calculated_offset);
